...
class ProjectController extends Controller
{
    ...
    public function submitFeedback(Request $request)
    {
        ...
        \Log::info('Feedback Submission:', $request->all());
    
        // Loop through ratings and save feedback
        foreach ($request->ratings as $memberId => $rating) {
            $feedback = new ProjectFeedback();
            $feedback->project_id = $request->project_id;
            $feedback->member_id = $memberId;
            $feedback->rating = $rating;
            $feedback->comment = $request->comments[$memberId] ?? null;
    
            // Debugging: Log each feedback entry
            \Log::info('Feedback Entry:', $feedback->toArray());
    
            // Attempt to save the feedback and catch errors
            try {
                $feedback->save();
            } catch (\Exception $e) {
